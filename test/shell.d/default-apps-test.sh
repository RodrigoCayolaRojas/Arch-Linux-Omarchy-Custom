...
if OMARCHY_TEST_REAL_BROWSER_INSTALL=true OMARCHY_TEST_INSTALL_FAIL=true \
  omarchy-default-browser --install firefox >"$test_tmp/browser-package-failure" 2>&1; then
  fail "failed Firefox package installation returns an error"
fi
[[ $(omarchy-default-browser) == "zen" ]] || fail "failed Firefox package installation preserves the default browser"
[[ ! -e $installed_dir/firefox ]] || fail "failed Firefox package installation does not mark it installed"
pass "failed browser package installation preserves the current default"

if OMARCHY_TEST_REAL_BROWSER_INSTALL=true OMARCHY_TEST_SETUP_FAIL=sudo \
  omarchy-default-browser --install firefox >"$test_tmp/browser-install-failure" 2>&1; then
  fail "failed Firefox setup returns an error"
fi
[[ $(omarchy-default-browser) == "zen" ]] || fail "failed Firefox setup preserves the default browser"
grep -Fq 'Installing Firefox' "$test_tmp/browser-install-failure" ||
  fail "failed Firefox setup keeps progress visible in the terminal"
pass "failed browser setup preserves the current default"
