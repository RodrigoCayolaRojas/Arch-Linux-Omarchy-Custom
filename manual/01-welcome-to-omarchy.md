...
Omarchy is an [omakase](https://manuals.omamix.org/3/omacom/76/omakase-computing) Linux distribution based on [Arch](https://archlinux.org/), the tiling window manager [Hyprland](https://hypr.land/), and the desktop construction-kit [Quickshell](https://quickshell.org/). It ships with a polished, focused base — Neovim, a full terminal workflow, screen recording, OCR, file sharing, and the visual environment that ties them together. Everything else, from browsers to office suites, is a deliberate, easy install from the Omarchy menu.
